...
(Note: don't use the password hashing function twice! Instead, with a modern password hashing function derive a 64-byte output and split it into two different 32-byte keys, or if you're stuck with PBKDF2, use HKDF to derive two keys from a single 32-byte output of PBKDF2).
